...
int nvs_config_load(app_config_t *config)
{
    ...
    nvs_get_str(handle, NVS_KEY_WIFI_SSID, config->wifi_ssid, &length);

    length = sizeof(config->wifi_pass);
    nvs_get_str(handle, NVS_KEY_WIFI_PASS, config->wifi_pass, &length);

    length = sizeof(config->gateway_ip);
    nvs_get_str(handle, NVS_KEY_GATEWAY_IP, config->gateway_ip, &length);

    length = sizeof(config->node_id);
    nvs_get_str(handle, NVS_KEY_NODE_ID, config->node_id, &length);

    length = sizeof(config->room_id);
    nvs_get_str(handle, NVS_KEY_ROOM_ID, config->room_id, &length);

    nvs_get_u8(handle, NVS_KEY_PROVISIONED, &config->provisioned);
...
int nvs_config_save(const app_config_t *config)
{
    ...
    nvs_set_str(handle, NVS_KEY_WIFI_PASS, config->wifi_pass);
    nvs_set_str(handle, NVS_KEY_GATEWAY_IP, config->gateway_ip);
    nvs_set_str(handle, NVS_KEY_NODE_ID, config->node_id);
    nvs_set_str(handle, NVS_KEY_ROOM_ID, config->room_id);
    nvs_set_u8(handle, NVS_KEY_PROVISIONED, config->provisioned);
