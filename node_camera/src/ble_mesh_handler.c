                                      ...
                                      esp_ble_mesh_model_cb_param_t *param)
{
    if (event == ESP_BLE_MESH_MODEL_OPERATION_EVT) {
        if (param->model_operation.opcode == ESP_BLE_MESH_VND_MODEL_OP_SET_CONFIG) {
            ...
            ESP_LOGI(TAG,
                     "Parsed camera config: gateway=%s:%u node=%s room=%s snapshot=%u rtsp=%u",
                     config.gateway_ip, (unsigned)config.gateway_port,
                     config.node_id, config.room_id,
                     (unsigned)config.snapshot_port, (unsigned)config.rtsp_port);
